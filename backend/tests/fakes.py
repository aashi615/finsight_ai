...
class FakeLLMProvider:
    ...
    async def complete_json(self, prompt: str, payload: dict, *, agent: str, max_output_tokens: int, force_fallback: bool = False) -> dict:
        ...
        if payload.get("evidence_manifest"):
            return {"executive_summary": "Based on available data, the evidence indicates monitored developments.", "company_overview": "Company overview is based on available data.", "market_analysis": market_analysis, "news_analysis": "News coverage indicates recent themes.", "growth_catalysts": claim, "key_risks": claim, "key_opportunities": claim, "competitive_landscape": "Competitive positioning is based on available data.", "valuation": "Valuation cannot be determined from available data.", "conclusion": "Based on available data, monitor further developments.", "evidence_ids": evidence_ids, "confidence": 0.6, "generated_at": datetime.now(timezone.utc).isoformat()}
        return {"executive_summary": "Based on available data, the evidence indicates monitored developments.", "company_overview": "Company overview is based on available data.", "market_analysis": market_analysis, "news_analysis": "News coverage indicates recent themes.", "growth_catalysts": claim, "key_risks": claim, "key_opportunities": claim, "competitive_landscape": "Competitive positioning is based on available data.", "valuation": "Valuation cannot be determined from available data.", "conclusion": "Based on available data, monitor further developments.", "evidence": evidence, "confidence": 0.6, "generated_at": datetime.now(timezone.utc).isoformat()}
